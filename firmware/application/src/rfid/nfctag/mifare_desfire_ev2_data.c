...
uint8_t *mifare_desfire_ev2_data_factory(uint8_t *buffer, tag_specific_type_t type) {
    ...
    if (fds_is_exists(map_info.id, map_info.key)) {
        ...
        if (!buffer) {
            uint8_t *existing;
            fds_read_sync(map_info.id, map_info.key, NULL, (uint8_t **)&existing);
    ...
    if (!fds_write_exists(map_info.id, map_info.key)) {
        ...
        if (ret) {
            ...
            return buffer ? buffer : factory_data;
        }
    }
...
int desfire_ev2_establish_secure_channel_aes(uint8_t slot, tag_data_buffer_t *buffer) {
    ...
    slot_config->communication_mode = NFC_AES_HIGH_RATE;
    slot_config->encryption_cipher = ENC_CIPHER_AES_GCM;
...
int desfire_ev2_establish_secure_channel_ccm(uint8_t slot, tag_data_buffer_t *buffer) {
    ...
    tag_slot_config_t *slot_config = get_slot_by_index(slot);
    slot_config->communication_mode = NFC_AES_HIGH_RATE;
    slot_config->encryption_cipher = ENC_CIPHER_AES_CCM;
...
void desfire_ev2_calc_auth_response_aes(uint8_t *key_encrypted, uint8_t *mac,
    ...
    uint8_t record_size = buffer->length > 0 ? (buffer->length >> 4) : 0;
    ...
    memcpy(auth_response, &mac_bytes[0], 4);
}

...
void em4305_field_induction_handler(void) {
    ...
    uint8_t *buffer = get_buffer_by_tag_type(TAG_TYPE_EM4305);
...
uint8_t em4305_parse_sectors(const uint8_t *uid, uint16_t uid_len) {
    ...
    uint8_t *data_start = &uid[uid_len - 1];  // Last byte contains sector count
 ...
 */
#define EM4305_32BIT    4      // 32-bit basic ID
#define EM4305_64BIT    8      // 64-bit extended ID

...
/** Maximum relay hop delay (µs) - must be < reader detection threshold */
...
typedef struct {
    ...
    uint32_t response_deadline_us;   // Deadline for relay response (µs)
...
int relay_core_relay_challenge(uint8_t *challenge, uint16_t challenge_len) {
    ...
    if (g_relay_session.last_challenge_ts == 0) {
        ...
        NRF_LOG_DEBUG("Stored challenge timestamp %dµs", g_relay_session.last_challenge_ts);
...
int relay_core_relay_response(uint8_t *response, uint16_t response_len) {
    ...
    uint32_t response_delay_us = nrf_rand() % 500UL + 100UL;  // 100-600µs
...
bool relay_core_activate_hop(uint8_t hop_index) {
    ...
    NRF_LOG_DEBUG("Activated antenna hop %d with deadline %dµs", 
